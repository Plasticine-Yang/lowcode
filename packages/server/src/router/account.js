...
const { jwtToJson } = require(path.join(__dirname, '../utils/jwt'))

const generateResBody = require(path.join(
  __dirname,
  '../utils/generate_res_body',
))
const conform = require(path.join(__dirname, '../utils/conform'))

const { sign_up, sign_in, userinfo_change, userinfo_get } = require(path.join(
  __dirname,
  '../database',
))
...
module.exports = async router => {
  ...
  router.post('/account/sign_up', async ctx => {
    ...
    const errMessage =
      'router/account/sign_up..., sign_up body parameter excepted a string'
    // 校验, 如果不符合断言返回错误
  ...
  router.post('/account/sign_in', async ctx => {
    ...
    const body = ctx.request.body
    const errMessage =
      'router/account/sign_in..., sign_in body parameter excepted a string'
    conform(body, shape, errMessage)
  ...
  router.patch('/account/userinfo/change', async ctx => {
    ...
    if (!token) {
      ...
      jwtToJson(ctx, token)
      const { errMessage, errorCode } = ctx.request.body
  ...
  router.post('/account/userinfo', async ctx => {
    console.log('account/userinfo--101', ctx.request.body)
    const token = ctx.request.body.accessToken
    if (!token) {
      ctx.response.body = generateResBody(40000, '请携带 token')
    } else {
