    ...
    (hashCollection, userCollection) =>
        async ({ componentJson, decoded }) => {
            ...
            if (await checkHash(username, hash, hashCollection)) {
                const info = await findInfoFromCollectionByUsername(userCollection, username)
                let { componentJsonList } = info
                componentJsonList = Array.from(componentJsonList)
                // 去除空对象
                const newList = 
                        !isEmpty(componentJson) 
                            ? [...componentJsonList, componentJson] 
                            : componentJsonList
                ...
                // 成功更新
                userCollection.update({username}, {"$set": {componentJsonList: newList}})
                return newInfo
            }
        }